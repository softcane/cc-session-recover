...
require_file ".claude/commands/session-recover.md"
...
require_text ".claude/auto-continue.md" "session-recover.js check"
require_text ".claude/auto-continue.md" "git status --short"
require_text ".claude/auto-continue.md" "Read .claude/auto-continue.md and follow it."
require_text ".claude/auto-continue.md" "DONE"
require_text ".claude/commands/session-recover.md" "Read .claude/auto-continue.md and follow it."
require_text ".claude/commands/session-recover.md" "Auto-recovery is ON"
...
require_text "README.md" "/session-recover"
require_text "README.md" "cc-session-recover watch"
...
if ! jq -e '(.hooks | keys == ["StopFailure"])' "$ROOT/.claude/settings.example.json" >/dev/null; then
  fail ".claude/settings.example.json must contain only the StopFailure hook"
fi
...
require_text "lib/recovery.js" "function checkMarker"
require_text "lib/recovery.js" "function clearMarker"
...
for deleted in \
  ...
  ".claude/hooks/remind-on-prompt.sh"; do
